...
"""Load file"""
path = '/Users/kat/DATA/FIRNPY_DATA/snowmelt.FGRN055_1957-2020_BN_RACMO2.3p2_Monthly.nc'

dfile = Dataset(path, "r", format="NETCDF4")
x = np.array(dfile.variables['lon'][:])
y = np.array(dfile.variables['lat'][:])
var = np.array(dfile.variables['snowmelt'][:])
ftime = np.array(dfile.variables['time'][:]) #hours since 1957-09-15 12:00:00
...
path_to_outfile = '/Users/kat/DATA/FIRNPY_DATA/snowmelt_monthly_polar_FGRN055_1957-2020_BN_RACMO23p2.nc'
...
var_monthly = data_file.createVariable('snowmelt', np.float64, ('time', 'lat', 'lon',))
...
var_monthly.description = 'Snowmelt from RACMO'
var_monthly.units = 'mm w.e. per year'
var_monthly.source = 'Original snowmelt from RACMO, coordinates converted to polar by KM Sejan'
